  ...
  return (
    <div className="card card-pad">
      ...
      {data.length === 0 ? (
        ...
        <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="180" preserveAspectRatio="xMidYMid meet">
          <defs>
          ...
          {points.map((p, i) => (
            <circle key={`dot-${i}`} cx={p[0]} cy={p[1]} r="3.5" fill="var(--surface)" stroke={color} strokeWidth="2" />
          ))}
          {data.map((d, i) => (
            <text
              key={`label-${i}`}
